# InferGate — resume & interview notes
...
> **InferGate — OpenAI-compatible LLM inference gateway** · *Python, FastAPI, Redis, Prometheus/Grafana, Docker*
...
- *"At AWS I built the GPU-to-CPU KV-cache reuse layer for one vLLM deployment;
  InferGate generalizes that idea to a multi-backend gateway with response-level
